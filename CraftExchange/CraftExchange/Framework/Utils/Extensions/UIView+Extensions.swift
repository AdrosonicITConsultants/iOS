...
extension UIView {
    ...
    func showChatHeaderDetailsView(controller: UIViewController, chat: Chat) {
        if let _ = self.viewWithTag(133) {
            ...
            if chat.productTypeId != nil {
                initiationView?.productType.text = chat.productTypeId!
            }else {
                 initiationView?.productType.text = "Custom design"
            }
           
            if chat.orderAmount != nil{
                 initiationView?.orderAmount.text = "Order amount: ₹" + chat.orderAmount!
            }else {
                 initiationView?.orderAmount.text = "Order amount: Not finalized"
            }
