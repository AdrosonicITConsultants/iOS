...
public class OfflineRequestManager: NSObject, NSCoding {
    ...
    public var reachabilityManager = try? Reachability()//NetworkReachabilityManager()
    ...
    private func setup() {
        do {
            reachabilityManager?.whenReachable = { [unowned self] status in
                self.connected = true
            }
            reachabilityManager?.whenUnreachable = { [unowned self] status in
    ...
    private func shouldAttemptRequest(_ request: OfflineRequest) -> Bool {
        ...
        if let manager = reachabilityManager {
            reachable = manager.connection != .unavailable
        }
    ...
    public func saveToDisk() {
        guard let path = OfflineRequestManager.fileURL(fileName: fileName) else { return }
