//
//  AdminUploadProductController.swift
//  CraftExchange
//
//  Created by Kiran Songire on 28/10/20.
...
class AdminUploadProductViewModel {
...
class AdminUploadProductController: FormViewController {
    var currentState: AdminNewProductState?
    var doneStates: [AdminNewProductState] = []
    lazy var viewModel = AdminUploadProductViewModel()
    ...
    override func viewDidLoad() {
        ...
        if let productObj = product {
            ...
            viewModel.prodDescription.value = productObj.productSpec ?? productObj.productDesc ?? ""
            downloadProdImages()
        }else {
            let rightButtonItem = UIBarButtonItem.init(title: "Select artisan brand", style: .plain, target: self, action: #selector(saveClicked))
        ...
        form
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.addPhotos.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
            ...
            <<< RoundedButtonViewRow("Next1") {
                ...
                $0.cell.tag = AdminNewProductState.addPhotos.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.addGeneralDetails.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< RoundedActionSheetRow() {
                ...
                //            row.cell.lengthTextField.textColor = .black
                //            row.cell.widthTextField.textColor = .black
                //            row.cell.lengthTextField.backgroundColor = .white
                //            row.cell.widthTextField.backgroundColor = .white
                ...
                //            row2.cell.lengthTextField.textColor = .black
                //            row2.cell.widthTextField.textColor = .black
                //            row2.cell.lengthTextField.backgroundColor = .white
                //            row2.cell.widthTextField.backgroundColor = .white
                ...
                let section = self.form.sectionBy(tag: "\(AdminNewProductState.gsm.rawValue)")
            ...
            <<< RoundedButtonViewRow("Next2") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.addGeneralDetails.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            ...
            <<< RoundedButtonViewRow("Next3") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.selectWeaveType.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.selectWarpWeftYarn.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< RoundedButtonViewRow("Next4") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.selectWarpWeftYarn.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.reedCount.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< ImageViewRow() {
                $0.cell.height = { 120.0 }
                $0.cell.cellImage.image = UIImage(named: "Reed Count-1")
                $0.hidden = true
            }
            <<< RoundedActionSheetRow() {
                $0.tag = "ReedCountRow"
            ...
            <<< RoundedButtonViewRow("Next5") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.reedCount.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.dimensions.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< ImageViewRow() {
                $0.cell.height = { 120.0 }
                $0.cell.cellImage.image = UIImage(named: "Dimension white")
                $0.hidden = true
            }
            <<< lengthWidthRow() {
                ...
                //            cell.lengthTextField.textColor = .black
                //            cell.widthTextField.textColor = .black
                //            cell.lengthTextField.backgroundColor = .white
                //            cell.widthTextField.backgroundColor = .white
            ...
            <<< RoundedButtonViewRow("Next6") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.dimensions.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            ...
            <<< RoundedButtonViewRow("Next7") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.washCare.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.availability.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< RoundedButtonViewRow("Next8") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.availability.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.weight.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< ImageViewRow() {
                $0.cell.height = { 60.0 }
                $0.cell.cellImage.image = UIImage(named: "Icon weight white")
                $0.hidden = true
            }
            <<< TextRow() {
                $0.tag = "ProdWeightRow"
                ...
                $0.cell.textField.backgroundColor = .black
                $0.cell.textField.textColor = .white
             //   $0.cell.textField.color
                $0.cell.textField.attributedPlaceholder = NSAttributedString(string: "Enter weight".localized,
            ...
            <<< RoundedButtonViewRow("Next9") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.weight.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.gsm.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< ImageViewRow() {
                $0.cell.height = { 120.0 }
                $0.cell.cellImage.image = UIImage(named: "GSM white")
                $0.hidden = true
            }.cellUpdate({ (cell, row) in
                if self.viewModel.prodCategory.value?.prodCatDescription == "Fabric" {
                    cell.height = { 60.0 }
            ...
            <<< RoundedButtonViewRow("Next10") {
                ...
                $0.cell.buttonView.setTitle("Next".localized, for: .normal)
                $0.cell.buttonView.setImage(UIImage.init(named: "pencil"), for: .normal)
                $0.cell.tag = AdminNewProductState.gsm.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
            }
            +++ Section(){ section in
                section.tag = "\(AdminNewProductState.addDescription.rawValue)"
                let ht: CGFloat = 60.0
                section.header = {
                    var header = HeaderFooterView<UIView>(.callback({
            ...
            <<< RoundedButtonViewRow("Next11") {
                ...
                $0.cell.tag = AdminNewProductState.addDescription.rawValue
                $0.cell.height = { 80.0 }
                $0.cell.delegate = self
                $0.hidden = true
        }
    ...
    func createWashCareSection() -> Section {
        let washCareSection = Section(){ section in
            section.tag = "\(AdminNewProductState.washCare.rawValue)"
            let ht: CGFloat = 60.0
            section.header = {
                var header = HeaderFooterView<UIView>(.callback({
        ...
        if currentState?.rawValue != tag {
            if tag == 9 {
                if viewModel.prodCategory.value?.prodCatDescription == "Fabric" {
                    currentState = AdminNewProductState.init(rawValue: tag)
                }else {
                    currentState = AdminNewProductState.init(rawValue: tag+1)
                }
            }else {
                currentState = AdminNewProductState.init(rawValue: tag)
            }
    ...
    func evaluateDot(section: Section) {
        ...
        if section.tag == "\(AdminNewProductState.addPhotos.rawValue)" {
            circle?.backgroundColor = self.viewModel.productImages.value?.count ?? 0 > 0 ? UIColor().CEGreen() : .red
        }else if section.tag == "\(AdminNewProductState.addGeneralDetails.rawValue)" {
        ...
        }else if section.tag == "\(AdminNewProductState.selectWeaveType.rawValue)" {
            circle?.backgroundColor = self.viewModel.prodWeaveType.value?.count ?? 0 > 0 ? UIColor().CEGreen() : .red
        }else if section.tag == "\(AdminNewProductState.selectWarpWeftYarn.rawValue)" {
            //Warp X Weft X Yarn
        ...
        }else if section.tag == "\(AdminNewProductState.reedCount.rawValue)" {
            //reed count
            circle?.backgroundColor = viewModel.reedCount.value != nil ? UIColor().CEGreen() : .red
        }else if section.tag == "\(AdminNewProductState.dimensions.rawValue)" {
            //dimensions
        ...
        }else if section.tag == "\(AdminNewProductState.gsm.rawValue)" {
            //gsm
        ...
        }else if section.tag == "\(AdminNewProductState.addDescription.rawValue)" {
            //prod description
            if viewModel.prodCategory.value?.prodCatDescription == "Fabric" {
        ...
        }else if section.tag == "\(AdminNewProductState.washCare.rawValue)" {
            // wash & care
            circle?.backgroundColor = self.viewModel.prodCare.value?.count ?? 0 > 0 ? UIColor().CEGreen() : .red
        }else if section.tag == "\(AdminNewProductState.weight.rawValue)" {
            //prod weight
        ...
        }else if section.tag == "\(AdminNewProductState.availability.rawValue)" {
            // availability
    ...
    func createSectionView(forStep:Int, title:String) -> UIView {
        ...
        stepTitle.textColor = .white
        stepTitle.text = title.localized
        view.addSubview(stepTitle)
        
        let arrow = UIButton.init(type: .custom)
        arrow.setImage(UIImage.init(systemName: "chevron.down"), for: .normal)
        arrow.addTarget(self, action: #selector(expandSectionSelected(sender:)), for: .touchUpInside)
        arrow.tintColor = #colorLiteral(red: 0.8862745098, green: 0.8862745098, blue: 0.8862745098, alpha: 1)
        arrow.tag = forStep-1
...
extension AdminUploadProductController: UITextViewDelegate {
...
extension AdminUploadProductController: ButtonActionProtocol, DimensionCellProtocol, LengthWidthCellProtocol, AvailabilityCellProtocol, ToggleButtonProtocol {
            ...
            if let weave = Weave.getWeaveType(searchId: tag) {
                if self.viewModel.prodWeaveType.value?.contains(weave) ?? false {
                    if let index = self.viewModel.prodWeaveType.value?.firstIndex(of: weave) {
                        ...
                        print(self.viewModel.prodWeaveType.value)
                    }
                }else {
    ...
    func customButtonSelected(tag: Int) {
        ...
        if tag == AdminNewProductState.addDescription.rawValue {
            self.viewModel.saveProductSelected?()
        }
    }
...
extension AdminUploadProductController: UICollectionViewDelegate, UICollectionViewDataSource, AddImageProtocol {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if currentState == AdminNewProductState.selectWarpWeftYarn {
            return 3
        }
...
extension AdminUploadProductController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension AdminUploadProductController: PhotoEditorDelegate {
...
extension AdminUploadProductController: OfflineRequestManagerDelegate {
